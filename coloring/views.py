...
def claimed(request, username =""):
  ...
  if request.POST: 
    ...
    for posting in user_claimed:
      ...
      post_info = [new_post.item_name, new_post.qty, new_post.qty_units, new_post.description, new_post.listing_user.username, json.dumps(new_post.unopened), json.dumps(new_post.og_packaging), json.dumps(new_post.store_bought), json.dumps(new_post.homemade),json.dumps(new_post.listing_user.verified),json.dumps(new_post.best_by, indent=4, sort_keys=True, default=str)]
